...
class EarthNoise: NoiseProtocol {

    let ContNoise: Perlin3D
    let MBiomeNoise: Perlin3D
    let MountainNoise: Perlin3D
    
    init() {
        ContNoise = Perlin3D(seed: "Cont")
    ...
    func value(pos: simd_double3) -> Double {
        ...
        let ContGrad = max(min(ContNoise.noise(x: x, y: y, z: z) - 0.1, 0.6), -0.6) / 5
        let Cont = min(max(ContGrad, -0.01), 0.005)
        let MBiome = min(max(0.2 - abs(MBiomeNoise.noise(x: x / 2, y: y / 2, z: z / 2)), 0), 0.05) * 8 * 10 * max(ContGrad, 0)
        let Mountain = max((MountainNoise.noise(x: x * 15, y: y * 15, z: z * 15) + 1), 0) * 0.06;
        return 1 + Cont + MBiome * Mountain
    }
